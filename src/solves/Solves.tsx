...
function EmptyDetail() {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 p-8 text-center">
      ...
      <p className="text-sm text-zinc-500">Select a solve to watch its status.</p>
    </div>
  )
}
...
export function SolvesDesktop() {
  ...
  // Land on the first solve so the pane is never empty; keep the user's pick stable
  // as live data streams in, but drop a selection that vanished.
  useEffect(() => {
    if (items.length === 0) {
      if (selectedId !== null) setSelectedId(null)
      return
    }
    if (!selectedId || !items.some((t) => t._id === selectedId)) setSelectedId(items[0]._id)
  }, [items, selectedId])
  ...
  return (
    <div className="mx-auto grid min-h-0 w-full max-w-7xl flex-1 grid-cols-[22rem_minmax(0,1fr)] grid-rows-1 gap-4 px-3 py-4">
      ...
      <section className="flex min-h-0 flex-col overflow-hidden rounded-lg border border-zinc-800 bg-zinc-950/70">
        {selected ? <SolveDetail t={selected} /> : <EmptyDetail />}
      </section>
    </div>
  )
}
